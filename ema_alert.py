...
def detect_sr_levels(candles, current_price, tolerance_pct=0.002, min_touches=2, max_levels=3):
    """Detecte les niveaux S/R par clustering des sommets et creux."""
...
def get_price_decimals(symbol):
    if symbol.startswith("cry") or symbol in ("BOOM1000","CRASH1000","BOOM500","CRASH500",
                                               "BOOM300N","CRASH300N","R_10","R_25","R_75",
                                               "STPINDEX","OTC_NDX"):
        return 2
    return 5
...
async def check_alert_group(symbol, granularity, label, alerts, state):
    ...
    closed_for_sr = candles[:-1]  # toutes les bougies fermees
