...
def escape_md(text):
    """Echappe les caracteres speciaux pour MarkdownV2 Telegram."""
    special = r"\_*[]()~`>#+-=|{}.!"
    return "".join(f"\{c}" if c in special else c for c in str(text))


def send_telegram(text, reply_markup=None, parse_mode=None):
...
def symbol_display_name(symbol, config=None):
    """Retourne le libellé affichable d'un symbole.

    Cherche d'abord dans les symboles personnalisés de la config (car un
    même code peut théoriquement être ajouté en custom même s'il existe
    déjà côté AVAILABLE_SYMBOLS), puis dans la liste par défaut, sinon
    retourne le code brut tel quel."""
    if config:
        for s in config.get("custom_symbols", []):
            if s["code"] == symbol:
                return s["label"]
    return SYMBOL_CODE_TO_LABEL.get(symbol, symbol)


def format_alert_line(alert, config):
    status = "✅" if alert.get("enabled", True) else "⏸"
    return (
        f"#{alert['id']} {symbol_display_name(alert['symbol'], config)} • "
...
def format_alerts_list(config):
    ...
    return "📋 Alertes\n\n" + "\n".join(format_alert_line(a, config) for a in alerts_sorted)
...
def parse_alert_id_arg(parts, command_example):
    """Parse et valide l'ID d'alerte passé en argument d'une commande
    (/pause, /resume, /delete). Factorise la logique répétée dans les
    trois commandes.

    Returns:
        (alert_id, error_message) — error_message vaut None si le parsing
        a réussi ; sinon alert_id vaut None et error_message est prêt à
        être envoyé tel quel via send_telegram().
    """
    if len(parts) < 2:
        return None, f"❌ Précise l'ID de l'alerte. Exemple : {command_example}"
    try:
        return int(parts[1]), None
    except ValueError:
        return None, f"❌ Format invalide. Exemple : {command_example}"


async def handle_text_command(config, text):
    parts = text.split()
    cmd = parts[0].lower()
    ...
    if cmd in ("/start", "/help"):
        ...
        alert_id, error = parse_alert_id_arg(parts, "/pause 3")
        if error:
            send_telegram(error)
        else:
            alert = find_alert(config, alert_id)
        ...
        alert_id, error = parse_alert_id_arg(parts, "/resume 3")
        if error:
            send_telegram(error)
        else:
            alert = find_alert(config, alert_id)
        ...
        alert_id, error = parse_alert_id_arg(parts, "/delete 3")
        if error:
            send_telegram(error)
        else:
            alert = find_alert(config, alert_id)
        ...
        if pending.get("step") == "custom_label":
            ...
            try:
                test_candles = await fetch_candles(custom_code, 3600, 10)
                if not test_candles:
...
def handle_new_alert_callback(config, chat_id, message_id, data_parts):
    ...
    if action == "symbol" and len(data_parts) >= 3:
        ...
        edit_telegram_message(
            ...
            message_id,
            f"➕ Nouvelle alerte\n\n"
            f"Symbole : {symbol_display_name(symbol, config)} ✅\n\n"
    ...
    if action == "gran" and len(data_parts) >= 4:
        ...
        try:
            ...
            edit_telegram_message(chat_id, message_id, "❌ Intervalle invalide, relance /new.")
            config["pending"] = None
            return
        if granularity not in SECONDS_TO_LABEL:
            edit_telegram_message(chat_id, message_id, "❌ Intervalle invalide, relance /new.")
            config["pending"] = None
            return
        # Label dérivé côté serveur (pas depuis le callback_data) pour garantir
        # la cohérence avec la granularité, même si le client envoie autre chose.
        label = SECONDS_TO_LABEL[granularity]
        config["pending"] = {
            "step": "ema",
        ...
        edit_telegram_message(
            ...
            message_id,
            f"➕ Nouvelle alerte\n\n"
            f"Symbole : {symbol_display_name(pending['symbol'], config)} ✅\n"
            f"Intervalle : {label} ✅\n\n"
            f"3/3 — Choisis l'EMA :",
    ...
    if action == "ema" and len(data_parts) >= 3:
        ...
        try:
            ...
            edit_telegram_message(chat_id, message_id, "❌ EMA invalide, relance /new.")
            config["pending"] = None
            return
        if ema not in AVAILABLE_EMA_PERIODS:
            edit_telegram_message(chat_id, message_id, "❌ EMA invalide, relance /new.")
            config["pending"] = None
            return
        symbol = pending["symbol"]
        ...
        if alert_already_exists(config, symbol, granularity, ema):
            edit_telegram_message(
                ...
                f"{symbol_display_name(symbol, config)} • {label} • EMA{ema}",
            )
            return
...
async def process_updates(config):
    """Lit les messages et boutons Telegram reçus depuis la dernière fois
    et met à jour la liste d'alertes en conséquence.

    Chaque update est traité dans un try/except individuel : si le
    traitement d'un update échoue, on log l'erreur et on continue avec les
    suivants au lieu de laisser planter tout le bot. Comme last_update_id
    est déjà avancé avant le traitement, un message qui provoque une
    exception n'est jamais retraité en boucle à l'exécution suivante."""
    ...
    for update in updates:
        ...
        try:
            ...
            await handle_text_command(config, text)
        except Exception as e:
            logger.error(f"❌ Erreur lors du traitement de l'update {update.get('update_id')} : {e}")
            continue
...
async def check_alert_group(symbol, granularity, label, alerts, state, config, daily_cache):
    """Vérifie un groupe d'alertes (même symbole + granularité).
    
    Gère les erreurs de connexion et les symboles invalides.

    daily_cache : dict partagé sur toute la durée de run_engine, qui évite
    de refetcher les bougies journalières (pour les pivots 1j) plusieurs
    fois pour un même symbole suivi sur plusieurs intervalles.
    """
    ...
    # Garde-fou minimal : il faut au moins 2 bougies fermées + la bougie en
    # cours pour calculer les Pivot Points (candles[-2]) sans IndexError.
    if len(candles) < 3:
        logger.warning(f"{symbol} ({label}) — Historique trop court ({len(candles)} bougies), abandon du groupe.")
        return

    max_period = max(a["ema"] for a in alerts)
    if len(candles) < max_period + 2:
        logger.warning(
            f"{symbol} ({label}) — Historique insuffisant pour l'EMA la plus longue du "
            f"groupe (besoin: {max_period + 2}, reçu: {len(candles)}). Les alertes dont "
            f"l'EMA ne peut pas être calculée seront ignorées individuellement plus bas ; "
            f"les autres alertes du groupe restent traitées normalement."
        )
    ...
    # Mis en cache par symbole (daily_cache est partagé entre tous les
    # groupes de run_engine) : un même symbole suivi sur plusieurs
    # intervalles (ex: XAUUSD 10min + 30min) ne déclenche qu'un seul fetch
    # journalier au lieu d'un par groupe.
    if symbol in daily_cache:
        pp_daily = daily_cache[symbol]
    else:
        pp_daily = None
        try:
        ...
        daily_cache[symbol] = pp_daily  # mis en cache même si None (pas de retry dans ce cycle)
    ...
    def build_trend_analysis(close, ema20, ema50, ema20_p, ema50_p):
        ...
        if ema20 is not None and ema20_p is not None:
            ...
            slope_str_esc = escape_md(slope_str)
            if abs(slope) > abs(ema20) * 0.0005:
                ...
                direction = "➡️ Faible"
            lines.append(f"• Pente EMA20        : {direction} \\({slope_str_esc}\\)")
    ...
    for alert in alerts:
        ...
        if touched and already_alerted_epoch != epoch:
            ...
            fmt = f"{ema_val:.{dec}f}"
            # Escape des caracteres speciaux MarkdownV2 pour les parties texte
            sym_name = escape_md(symbol_display_name(symbol, config))
            label_esc = escape_md(label)
            time_esc = escape_md(time_str)
            period_esc = escape_md(str(period))
            alert_id_esc = escape_md(str(alert["id"]))

            msg_parts = [
                f"🔔 {sym_name} \({label_esc}\) \- {time_esc}",
                f"⚡ Prix a touché l'EMA{period_esc} \(\~`{fmt}`\)",
                "",
            ...
            msg_parts += ["", sr_block, "", f"Alerte \#{alert_id_esc}"]
            msg = "\n".join(msg_parts)
...
async def run_engine(config, state):
    ...
    daily_cache = {}  # partagé entre tous les groupes de ce cycle (cf. check_alert_group)
    for (symbol, granularity), alerts in groups.items():
        label = alerts[0]["label"]
        await check_alert_group(symbol, granularity, label, alerts, state, config, daily_cache)
        # Sauvegarde incrémentale : si ce run est annulé (nouveau trigger,
        # timeout) avant d'avoir traité tous les groupes, les alertes déjà
        # envoyées dans CE cycle ne sont pas perdues -- elles sont déjà sur
        # disque. Seuls les groupes pas encore atteints seront re-vérifiés
        # au prochain cycle (quelques secondes/minutes plus tard).
        save_state(state)
        save_config(config)
...
async def main():
    ...
    try:
        await process_updates(config)
    except Exception as e:
        logger.error(f"❌ Erreur inattendue dans process_updates : {e}")
    save_config(config)          # sauvegarde pending + alertes CRUD immédiatement
    prune_state(config, state)

    # ✅ Timeout global : 150s, pour laisser de la marge à l'étape de
    # sauvegarde Git qui suit (le job GitHub a un timeout-minutes de 4 = 240s
    # au total : checkout + pip install + ce script + git push doivent tous
    # tenir dedans, pas seulement le script).
    try:
        logger.info("Lancement du moteur de vérification avec timeout de 150s")
        await asyncio.wait_for(run_engine(config, state), timeout=150)
    except asyncio.TimeoutError:
        logger.warning("⚠️ Moteur d'alertes dépassé (>150s), abandon pour ce cycle.")
    except Exception as e:
