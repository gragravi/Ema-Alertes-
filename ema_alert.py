...
def get_price_decimals(symbol):
    if symbol.startswith("cry") or symbol in ("BOOM1000","CRASH1000","BOOM500","CRASH500",
                                               "BOOM300N","CRASH300N","R_10","R_25","1HZ25V","R_50","R_75",
                                               "STPINDEX","OTC_NDX"):
        return 2
    return 5
...
async def check_alert_group(symbol, granularity, label, alerts, state, config):
...
async def run_engine(config, state):
    ...
    for (symbol, granularity), alerts in groups.items():
        ...
        await check_alert_group(symbol, granularity, label, alerts, state, config)
